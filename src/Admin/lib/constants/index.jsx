import { blue, gray, yellow } from "./classes";

// Sidebar Top Links
export const dashboardTopLink = [
  {
    key: "dashboard",
    label: "Dashboard",
    path: "dashboard",
    // img: require("../../images/dashboard.png"),
  },
  {
    key: "myUsers",
    label: "Users",
    path: "/users",
    // img: require("../../images/product.png"),
  },
  {
    key: "myForm",
    label: "Edit Form",
    path: "dashboard/form",
    // img: require("../../images/product.png"),
  },
  // {
  //   key: "addProducts",
  //   label: "Add Products",
  //   path: "dashboard/addproducts",
  //   img: require("../../images/add_products.png"),
  // },
  // {
  //   key: "viewSite",
  //   label: "View Site",
  //   path: "dashboard/viewsite",
  //   img: require("../../images/view_site.png"),
  // },
  // {
  //   key: "shopCart",
  //   label: "Shop Cart",
  //   path: "dashboard/shopcart",
  //   img: require("../../images/shop_cart.png"),
  // },
  // {
  //   key: "payment",
  //   label: "Payment",
  //   path: "dashboard/payment",
  //   img: require("../../images/payment.png"),
  // },
  // {
  //   key: "setting",
  //   label: "Setting",
  //   path: "dashboard/setting",
  //   img: require("../../images/setting.png"),
  // },
  // {
  //   key: "listingsManagement",
  //   label: "Listings Management",
  //   path: "dashboard/listingsmanagement",
  //   img: require("../../images/listing.png"),
  // },
];

// Sidebar Bottom Links
// export const dashboardBottomLink = [
  // {
  //   key: "Goto Website",
  //   label: "Goto Website",
  //   path: "/gotowebsite",
  //   img: require("../../images/goto.png"),
  // },
// ];

// // Dashboard Total Data
// export const dashTotalData = [
//   {
//     key: "Today's Sale",
//     title: "Today's Sale",
//     title_span: "₹5000",
//     img: require("../../images/sales.png"),
//     class: blue,
//   },
//   {
//     key: "Today's Earn",
//     title: "Today's Earn",
//     title_span: "₹4000",
//     img: require("../../images/earn.png"),
//     class: yellow,
//   },
//   {
//     key: "Our Client",
//     title: "Our Client",
//     title_span: "4000",
//     img: require("../../images/client.png"),
//     class: gray,
//   },
// ];

// // Dashboard Recent Data
// export const dashRecentData = [
//   {
//     key: "Cameron Williamson",
//     title: "Cameron Williamson",
//     title_span: "2 Minutes Ago",
//     transaction: "+₹5000",
//     img: require("../../images/cameron.png"),
//     class: blue,
//     bg: "bg-[#F4F4FA]",
//   },
//   {
//     key: "Wade Warren",
//     title: "Wade Warren",
//     title_span: "2 Minutes Ago",
//     transaction: "+₹5000",
//     img: require("../../images/wade.png"),
//     class: yellow,
//     border: "border-2",
//     border_color: "border-[#1861aa80]",
//   },
//   {
//     key: "Kristin Western",
//     title: "Kristin Western",
//     title_span: "2 Minutes Ago",
//     transaction: "+₹5000",
//     img: require("../../images/kristin.png"),
//     class: yellow,
//     border: "border-2",
//     border_color: "border-[#1861aa80]",
//   },
//   {
//     key: "Kathryn Murphy",
//     title: "Kathryn Murphy",
//     title_span: "2 Minutes Ago",
//     transaction: "+₹5000",
//     img: require("../../images/kathryn.png"),
//     class: yellow,
//     border: "border-2",
//     border_color: "border-[#1861aa80]",
//   },
//   {
//     key: "Cody Fisher",
//     title: "Cody Fisher",
//     title_span: "2 Minutes Ago",
//     transaction: "+₹5000",
//     img: require("../../images/cody.png"),
//     class: yellow,
//     border: "border-2",
//     border_color: "border-[#1861aa80]",
//   },
// ];

// // Dashboard History Table Title
// export const dashHistoryTableTitle = [
//   {
//     key: "Tracking ID",
//     title: "Tracking ID",
//   },
//   {
//     key: "Recipient",
//     title: "Recipient",
//   },
//   {
//     key: "Status",
//     title: "Status",
//   },
//   {
//     key: "Payment Method",
//     title: "Payment Method",
//   },
// ];

// // Dashboard History Table Data
// export const dashHistoryTableData = [
//   {
//     key: "Ipad Air 1",
//     tracking_id: "#12323645",
//     title: "Ipad Air",
//     img: require("../../images/ipad.png"),
//     status: "Pending",
//     payment: "Visa card 0236",
//   },
//   {
//     key: "Ipad Air 2",
//     tracking_id: "#85614858",
//     title: "Ipad Air",
//     img: require("../../images/ipad.png"),
//     status: "Completed",
//     payment: "Paypal 0263",
//   },
//   {
//     key: "Ipad Air 3",
//     tracking_id: "#98751531",
//     title: "Ipad Air",
//     img: require("../../images/ipad.png"),
//     status: "Pending",
//     payment: "Visa card 0254",
//   },
//   {
//     key: "Ipad Air 4",
//     tracking_id: "#15469716",
//     title: "Ipad Air",
//     img: require("../../images/ipad.png"),
//     status: "Completed",
//     payment: "Paypal 5452",
//   },
// ];

// // Dashboard Graph
// export const weekSummary = [
//   {
//     key: "Income",
//     title: "Income",
//     title_span: "30%",
//     img: require("../../images/green.png"),
//     gap: "gap-x-6",
//     class_left: "ml-4",
//   },
//   {
//     key: "Expense",
//     title: "Expense",
//     title_span: "46%",
//     img: require("../../images/blue.png"),
//     gap: "gap-x-6",
//     class: "mt-6",
//     class_left: "ml-2",
//   },
//   {
//     key: "Unknown",
//     title: "Unknown",
//     title_span: "10%",
//     img: require("../../images/yellow.png"),
//     gap: "gap-x-6",
//     class: "mt-6",
//     class_left: "ml-2",
//   },
// ];

// // ViewSite NavLinks
// export const NavLinks = [
//   {
//     key: "Go to Dashboard",
//     title: "Go to Dashboard",
//   },
//   {
//     key: "Home",
//     title: "Home",
//   },
//   {
//     key: "About",
//     title: "About",
//   },
//   {
//     key: "Products",
//     title: "Products",
//   },
//   {
//     key: "Contact",
//     title: "Contact",
//   },
// ];

// // ViewSite Photos
// export const Photos = [
//   {
//     key: "Photo1",
//     img: require("../../images/Noise_logo.jpeg"),
//   },
//   {
//     key: "Photo2",
//     img: require("../../images/Noise_logo.jpeg"),
//   },
//   {
//     key: "Photo3",
//     img: require("../../images/Noise_logo.jpeg"),
//   },
//   {
//     key: "Photo4",
//     img: require("../../images/Noise_logo.jpeg"),
//   },
// ];

// // ViewSite Bestseller
// export const BestSeller = [
//   {
//     img: require("../../images/ptron.png"),
//     brand: "PTron",
//     brand_span: "pTron Boom Ultima 4D dual Driver in-ear wired headphone",
//     strike_price: "₹1700/-",
//     final_price: "₹199/-",
//     discount: `(88% off)`,
//   },
//   {
//     img: require("../../images/ptron.png"),
//     brand: "PTron",
//     brand_span: "pTron Boom Ultima 4D dual Driver in-ear wired headphone",
//     strike_price: "₹1700/-",
//     final_price: "₹199/-",
//     discount: `(88% off)`,
//   },
//   {
//     img: require("../../images/ptron.png"),
//     brand: "PTron",
//     brand_span: "pTron Boom Ultima 4D dual Driver in-ear wired headphone",
//     strike_price: "₹1700/-",
//     final_price: "₹199/-",
//     discount: `(88% off)`,
//   },
//   {
//     img: require("../../images/ptron.png"),
//     brand: "PTron",
//     brand_span: "pTron Boom Ultima 4D dual Driver in-ear wired headphone",
//     strike_price: "₹1700/-",
//     final_price: "₹199/-",
//     discount: `(88% off)`,
//   },
//   {
//     img: require("../../images/ptron.png"),
//     brand: "PTron",
//     brand_span: "pTron Boom Ultima 4D dual Driver in-ear wired headphone",
//     strike_price: "₹1700/-",
//     final_price: "₹199/-",
//     discount: `(88% off)`,
//   },
//   {
//     img: require("../../images/ptron.png"),
//     brand: "PTron",
//     brand_span: "pTron Boom Ultima 4D dual Driver in-ear wired headphone",
//     strike_price: "₹1700/-",
//     final_price: "₹199/-",
//     discount: `(88% off)`,
//   },
// ];
// // ViewSite Feedback
// export const feedBack = [
//   {
//     img: require("../../images/cody.png"),
//     title: "Daniel Lane",
//     para: "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Dolor voluptate, deleniti adipisci incidunt.",
//   },
//   {
//     img: require("../../images/cody.png"),
//     title: "Daniel Lane",
//     para: "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Dolor voluptate, deleniti adipisci incidunt.",
//   },
//   {
//     img: require("../../images/cody.png"),
//     title: "Daniel Lane",
//     para: "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Dolor voluptate, deleniti adipisci incidunt.",
//   },
// ];

// // Cart Table Titles
// export const cartTableTitle = [
//   {
//     key: "Product Details",
//     title: "Product Details",
//   },
//   {
//     key: "Quantity",
//     title: "Quantity",
//   },
//   {
//     key: "Price",
//     title: "Price",
//   },
//   {
//     key: "Total",
//     title: "Total",
//   },
// ];

// // Cart Table Data
// export const cartTableData = [
//   {
//     key: "Ipad Air1",
//     title: "Ipad Air",
//     title_span: "Apple",
//     img: require("../../images/ipad1.png"),
//     price: "₹54900/-",
//     total: "₹54900/-",
//   },
//   {
//     key: "Ipad Air2",
//     title: "Ipad Air",
//     title_span: "Apple",
//     img: require("../../images/ipad1.png"),
//     price: "₹54900/-",
//     total: "₹54900/-",
//   },
//   {
//     key: "Ipad Air3",
//     title: "Ipad Air",
//     title_span: "Apple",
//     img: require("../../images/ipad1.png"),
//     price: "₹54900/-",
//     total: "₹54900/-",
//   },
//   {
//     key: "Ipad Air4",
//     title: "Ipad Air",
//     title_span: "Apple",
//     img: require("../../images/ipad1.png"),
//     price: "₹54900/-",
//     total: "₹54900/-",
//   },
//   {
//     key: "Ipad Air5",
//     title: "Ipad Air",
//     title_span: "Apple",
//     img: require("../../images/ipad1.png"),
//     price: "₹54900/-",
//     total: "₹54900/-",
//   },
// ];
